...
class LoadImagesAndLabels(Dataset):
    ...
    def check_cache_ram(self, safety_margin=0.1, prefix=""):
        ...
        if not cache:
            LOGGER.info(f"{prefix}{mem_required / gb:.1f}GB RAM required, " f"{mem.available / gb:.1f}/{mem.total / gb:.1f}GB available, " f"{'caching images ✅' if cache else 'not caching images ⚠️'}")
        return cache
