...
def export_openvino(file, metadata, half, int8, data, prefix=colorstr("OpenVINO:")):
    ...
    if int8:
        ...
        def gen_dataloader(yaml_path, task="train", imgsz=640, workers=4):
            ...
            dataloader = create_dataloader(data[task], imgsz=imgsz, batch_size=1, stride=32, pad=0.5, single_cls=False, rect=False, workers=workers)[0]
            return dataloader
...
def export_tfjs(file, int8, prefix=colorstr("TensorFlow.js:")):
    ...
    with open(f_json, "w") as j:  # sort JSON Identity_* in ascending order
        subst = re.sub(
            r'{"outputs": {"Identity.?.?": {"name": "Identity.?.?"}, ' r'"Identity.?.?": {"name": "Identity.?.?"}, ' r'"Identity.?.?": {"name": "Identity.?.?"}, ' r'"Identity.?.?": {"name": "Identity.?.?"}}}',
            r'{"outputs": {"Identity": {"name": "Identity"}, ' r'"Identity_1": {"name": "Identity_1"}, ' r'"Identity_2": {"name": "Identity_2"}, ' r'"Identity_3": {"name": "Identity_3"}}}',
            json,
        )
        j.write(subst)
    return f, None
...
def parse_opt(known=False):
    ...
    parser.add_argument("--weights", nargs="+", type=str, default=ROOT / "dnf.pt", help="model.pt path(s)")
